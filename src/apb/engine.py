...
SKIP_OPTIONS = ['provision', 'deprovision', 'bind', 'unbind', 'roles', 'all']
...
def generate_playbook_files(playbooks_path, roles_path, bindable, skip, apb_name):
    ...
    if 'provision' not in skip:
        # Create provision playbook
        write_playbook_file(playbooks_path, PROVISION_PLAYBOOK, template, apb_name, 'provision')
        if 'roles' not in skip:
            os.mkdir(os.path.join(roles_path, 'provision-' + apb_name))

    if 'deprovision' not in skip:
        # Create deprovision playbook
        write_playbook_file(playbooks_path, DEPROVISION_PLAYBOOK, template, apb_name, 'deprovision')
        if 'roles' not in skip:
            os.mkdir(os.path.join(roles_path, 'deprovision-' + apb_name))

    if 'bind' not in skip:
        # Create Bind playbook
        write_playbook_file(playbooks_path, BIND_PLAYBOOK, template, apb_name, 'bind')
        if 'roles' not in skip:
            os.mkdir(os.path.join(roles_path, 'bind-' + apb_name))

    if 'unbind' not in skip:
        # Create Unbind playbook
        write_playbook_file(playbooks_path, UNBIND_PLAYBOOK, template, apb_name, 'unbind')
        if 'roles' not in skip:
            os.mkdir(os.path.join(roles_path, 'unbind-' + apb_name))
...
def cmdrun_init(**kwargs):
    ...
    for skip_option in skip:
        if skip_option not in SKIP_OPTIONS:
            raise Exception(
                "Error: [%s] is not a valid option to skip. The following are skippable options: %s" %
                (skip_option, SKIP_OPTIONS)
            )
